...
public class Luby : Symbol
{
    ...
    void Buffer2(Symbol symbol)
    {
        if (symbol.symbol == eSymbol.Luby)
        {
            coin += 1;
        }
    }
    ...
    private void CheckAll(SymbolTurn[] symbolTurns)
    {
        foreach(SymbolTurn symbolTurn in symbolTurns)
        {
            Symbol symbol = symbolTurn.GetComponentInChildren<Symbol>();
            Buffer2(symbol);
        }
    }
}
